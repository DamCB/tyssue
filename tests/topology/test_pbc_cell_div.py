...
div_cell = sheet.face_df.index[
    (sheet.face_df["at_x_boundary"] == True) & (sheet.face_df["at_y_boundary"] == False)
].tolist()[0]
print(
    "test cell division on boundary begins (rember point is that in the original cell_div, cells on the boundary cannot divide) "
)
...
div_cell = sheet.face_df.index[
    (sheet.face_df["at_x_boundary"] == True) & (sheet.face_df["at_y_boundary"] == True)
].tolist()[0]
daughter = cell_division(sheet, div_cell, PlanarGeometry, angle=0.6)
sheet.face_df.loc[div_cell, "prefered_area"] = 1.0
sheet.face_df.loc[daughter, "prefered_area"] = 1.0
