...
from tyssue.geometry.cellcell_geometry import (
    scale,
    update_dcoords,
    update_length,
    update_all,
...
def test_scale():
    ...
    initial_cell_df = pd.DataFrame.from_dict(
        {
            "cell": [0, 1, 2],
            "x": [0.5, -1.0, 0.5],
    ...
    x10_cell_df = pd.DataFrame.from_dict(
        {
            "cell": [0, 1, 2],
            "x": [5.0, -10.0, 5.0],
            "y": [8.660000e00, -6.167906e-17, -8.6600000e00],
            "z": [-5.0, -5.0, -5.0],
            "is_alive": [True, True, True],
            "num_faces": [8, 8, 8],
            "vol": [2.598, 2.598, 2.598],
    ...
    eptm = Epithelium(
        "test_volume", datasets, config.geometry.bulk_spec(), coords=["x", "y", "z"]
    )
    ...
    assert all(
        (x10_cell_df[["x", "y", "z"]] - eptm.cell_df[["x", "y", "z"]]) ** 2 < 1e-16
    )
