...
def test_3cells():
    ...
    sgeom.update_dcoords(sheet, sheet.coords)
    sgeom.update_length(sheet, sheet.coords)
    np.testing.assert_allclose(sheet.je_df['length'], 1,
                               rtol=1e-3)

    sgeom.update_centroid(sheet, sheet.coords)
    np.testing.assert_array_almost_equal([0.5, -1., 0.5],
                                         sheet.cell_df['x'],
                                         decimal=3)

    sgeom.update_normals(sheet, sheet.coords)
    norms = np.linalg.norm(sheet.je_df[['nx', 'ny', 'nz']], axis=1)
    np.testing.assert_allclose(norms, 0.866, rtol=1e-3)

    sgeom.update_areas(sheet, sheet.coords)
