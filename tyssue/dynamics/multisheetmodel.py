...
class MultiSheetModel(SheetModel):
    ...
    def compute_gradient(cls, msheet, components=False):
        ...
        grads = [
            pd.DataFrame(0.0, index=sheet.vert_df.index, columns=sheet.coords)
            for sheet in msheet
        ]
        ...
        for sheet, (i, grad) in zip(msheet, enumerate(grads)):
            ...
            grad_i = (
                ...
                + sheet.sum_srce(grad_c)
                - sheet.sum_trgt(grad_c)
    ...
    def desmosome_gradient(msheet, grads):
        ...
        for i, sheet in enumerate(msheet):
            norm_factor = sheet.specs["settings"]["nrj_norm_factor"]
            upward = sheet.vert_df.eval("d_elasticity * (height - prefered_height)")
            grads[i]["z"] += upward / norm_factor

        for i, sheet in enumerate(msheet):
            norm_factor = sheet.specs["settings"]["nrj_norm_factor"]
            downward = sheet.vert_df.eval("u_elasticity * (prefered_depth - depth)")
...
def set_model(msheet, specs):
    ...
    for sheet in msheet:
        sheet.update_specs(specs, reset=True)  # MultiSheetModel.dimentionalize(specs),
    msheet[-1].vert_df["basal_shift"] = msheet[-1].vert_df.eval("z + prefered_depth")
    msheet[0].vert_df["basal_shift"] = msheet[0].vert_df.eval("z - prefered_height")
