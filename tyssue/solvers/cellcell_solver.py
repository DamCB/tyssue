...
def find_energy_min(ccmesh, geom, model, pos_idx=None, **settings_kw):
...
def approx_grad(ccmesh, geom, model):
    ...
    grad = optimize.approx_fprime(pos0, opt_energy, 1e-9, pos_idx, ccmesh, geom, model)
    return grad
...
def check_grad(ccmesh, geom, model):
    ...
    grad_err = optimize.check_grad(
        opt_energy, opt_grad, pos0.flatten(), pos_idx, ccmesh, geom, model
    )
    return grad_err
