...
class MultiSheetSolver(Solver):
    @classmethod
    def find_energy_min(cls, msheet, geom, model, pos_idxs, **settings_kw):
        settings = minimize_spec()
        settings.update(**settings_kw)

        pos0 = [
            sheet.vert_df.loc[pos_idx, sheet.coords].values.flatten()
    ...
    def set_pos(pos, pos_idxs, msheet):
        v_idxs = np.array([pos_idx.shape[0] for pos_idx in pos_idxs]).cumsum()
        for sheet, pos_chunk, pos_idx in zip(
            msheet, np.split(pos, v_idxs[:-1] * 3), pos_idxs
        ):
    ...
    def opt_grad(pos, pos_idxs, msheet, geom, model):
        ...
        grad_i = np.concatenate(
            [
                grad.loc[pos_idx].values.flatten()
                for grad, pos_idx in zip(grads, pos_idxs)
            ]
        )
        return grad_i
    ...
    def check_grad(cls, msheet, geom, model):
        ...
        pos_idxs = [
            sheet.vert_df[sheet.vert_df["is_active"] == 1].index for sheet in msheet
        ]
        pos0 = [
            sheet.vert_df.loc[pos_idx, sheet.coords].values.flatten()
            for sheet, pos_idx in zip(msheet, pos_idxs)
        ...
        grad_err = optimize.check_grad(
            cls.opt_energy, cls.opt_grad, pos0, pos_idxs, msheet, geom, model
        )
        return grad_err
